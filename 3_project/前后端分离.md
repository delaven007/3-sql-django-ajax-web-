...
​			3，Content-Type (认定为表单form提交)仅支持如下三种：

​					application/x-www-form-urlencoded   				:form表单提交

​					multipart/form-data												：图片上传，文件上传
...
1.FTP获取源码包 or 官网下载 tar.gz结尾安装包

2.将源码包粘贴至虚拟机

3.执行文件  tar  -zxvf django-cors-headers-3.0.2 解压

4.cd到解压目录 cd django-cors-headers-3.0.2/
...
```python
		...
		7, CORS_PREFLIGHT_MAX_AGE  默认 86400s(一天)
		8, CORS_EXPOSE_HEADERS  []
		9, CORS_ALLOW_CREDENTIALS  布尔值， 默认False          #是否获取cookie
```


# 4，RESTful -Representational State Transfer(资源状态转移)
